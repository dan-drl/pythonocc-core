...
%template(Storage_PArray) NCollection_Array1 <Handle_Standard_Persistent>;

%extend NCollection_Array1 <Handle_Standard_Persistent> {
    %pythoncode {
...
%extend NCollection_Array1 <Handle_Storage_CallBack> {
    %pythoncode {
    def __getitem__(self, index):
        if index + self.Lower() > self.Upper():
            raise IndexError("index out of range")
...
%extend NCollection_Array1 <Handle_Storage_Schema> {
    %pythoncode {
    def __getitem__(self, index):
        if index + self.Lower() > self.Upper():
            raise IndexError("index out of range")
...
typedef NCollection_Array1 <Handle_Standard_Persistent> Storage_PArray;
